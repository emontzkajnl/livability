...
define( 'HOUZEZ_THEME_VERSION', '4.2.2' );
...
if( ! function_exists('houzez_preload_property_featured_image') ) {
	function houzez_preload_property_featured_image() {
		...
		// Get appropriate image size based on layout
		$image_size = 'houzez-property-detail-gallery';
		...
		if ( $image_data && isset( $image_data[0] ) ) {
			...
			if ( $image_width && $image_height ) {
				echo ' imagesrcset="' . esc_attr( wp_get_attachment_image_srcset( $featured_image_id, $image_size ) ) . '"';
				echo ' imagesizes="(max-width: ' . esc_attr( $image_width ) . 'px) 100vw, ' . esc_attr( $image_width ) . 'px"';
			}
