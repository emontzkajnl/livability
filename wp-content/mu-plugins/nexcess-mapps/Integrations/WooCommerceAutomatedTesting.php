...
class WooCommerceAutomatedTesting extends Integration {
	...
	protected function getFilters() {
		return [
			...
			// Captcha plugins for WC add errors on the registration process. We need to stop them so the user can be created.
			[ 'woocommerce_registration_errors', [ $this, 'stopRegistrationErrors' ], PHP_INT_MAX, 3 ],
		];
	}
	 ...
	 *
	 * @throws WPErrorException If the reqeust to update a site fails.
	 */
	 ...
	 * @return WP_Error
	 */
	public function stopRegistrationErrors( $errors, $username, $email ) {
		// Check for our WCAT username and email.
		if ( false !== strpos( $username, 'nexcess_wc_automated_testing_' )
			&& false !== strpos( $email, 'wc-automated-testing+' )
			&& false !== strpos( $email, '@nexcess.net' ) ) {
