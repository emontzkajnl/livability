...
abstract class BaseGlobalInsightTypeGenerator extends BaseInsightTypeGenerator {
	...
	/**
	 * Converts the raw `globalData` items into `MetricMatrix` instances.
	 *
	 * @param string $metric Metric type to be used as cell values in the matrix.
	 *
	 * @return MetricMatrix[]
	 */
	public function generateGlobalDataMatrices( $metric = 'score' ) {
		return array_map(
			function( $global_data_one_day ) use ( $metric ) {
				return MetricMatrix::fromGlobalPerformanceData( $global_data_one_day, $metric );
			},
			$this->globalData
		);
	}
