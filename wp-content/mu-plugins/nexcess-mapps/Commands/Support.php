...
class Support extends Command {
	 ...
	 */
	public function __construct( PageCache $page_cache, Settings $settings, FeatureFlags $flags ) {
	 ...
	 * @param mixed[] $assoc_args Associative arguments.
	 */
	public function featureFlags( $args, $assoc_args ) {
		$action  = $args[0];
		$flag    = $args[1];
		$option  = $this->flags->getOption();
		$flags   = $option->all();
		$cohorts = [];
		if ( ! empty( $flags['cohorts'] ) ) {
		...
		switch ( $action ) {
			case 'enable':
				$option->set( $flag, 50 )->save();
				$option->set('cohorts', array_merge( (array) $option->get( 'cohorts' ), [
					$flag => 1,
				]))->save();
				WP_CLI::success( $flag . ' is now enabled.' );
				break;

			case 'disable':
				$option->set( $flag, 0 )->save();
				$option->set('cohorts', array_merge( (array) $option->get( 'cohorts' ), [
					$flag => 50,
				]))->save();
				WP_CLI::success( $flag . ' is now disabled.' );
				break;

			case 'get':
			default:
				if ( ! empty( $assoc_args['all'] ) ) {
					if ( empty( $cohorts ) || empty( $flags ) ) {
						WP_CLI::line( 'Feature flag data not yet available.' );
						return;
					}
					self::section( 'Flags (remote)', $flags, 'feature_flags' );
					self::section( 'Cohorts', $cohorts, 'feature_flags' );
					WP_CLI::line();
					return;
				}
				$enabled = intval( ! empty( $cohorts[ $flag ] ) ? $cohorts[ $flag ] : 0 ) < intval( ! empty( $flags[ $flag ] ) ? $flags[ $flag ] : 0 );
				self::output( $flag, $enabled, $enabled ? 'G' : 'R' );
				break;
		}
	}
	...
	public function settings( $args ) {
		$setting = $this->settings->getSetting( $args[0] );
		if ( is_null( $setting ) ) {
			WP_CLI::error( $args[0] . ' is not defined.' );
		}
		if ( is_bool( $setting ) ) {
