...
class MigrationCleaner {
	...
	public function clean() {
		...
		// Removes redundant config blocks from the config file.
		$this->removeBlocksFromConfig();
	...
	public function scanConfig() {
		$content   = $this->getConfigContent( $this->getConfigFilePath() );
		$patterns  = $this->getPatterns();
		$constants = [];

		// Iterate through each pattern.
		foreach ( $patterns as $group => $pattern_group ) {
			$constants = array_merge( $constants, array_fill_keys(
				$this->processPatternGroup( $content, $pattern_group ),
				sprintf( 'Constant is redundant and was defined in %s config block.', $group )
			) );
		}
	...
	public function scan() {
		return array_merge_recursive( $this->scanConfig(), $this->scanFileSystem() );
	}
	 ...
	 *
	 * @return array[] Leftover files, grouped by host.
	 *
	 ...
	 *
	 * @return array<string, string[]> Leftover config blocks, grouped by host.
	 */
	protected function getPatterns() {
		return [
			'Dreamhost' => [
				'/if[^{]*dream[^{]*website[^{]*\{([^}]*)\}/s', // Searching for .dream.website regex config block.
			],
		];
	}
	 ...
	 * @param array  $pattern_group
	 *
	 * @return string[]
	 */
	protected function processPatternGroup( $content, $pattern_group ) {
		$constants = [];

		foreach ( $pattern_group as $pattern ) {
			// Check if pattern matches.
			if ( preg_match( $pattern, $content, $matches ) ) {
				preg_match_all( '/define\(\'([^\']+)\'/', $matches[0], $const_matches );
				$constants = array_merge( $constants, $const_matches[1] );
			}
		}
	...
	protected function getConfigFilePath() {
		...
		if ( file_exists( ABSPATH . $wp_config ) ) {
			return ABSPATH . $wp_config;
		}

		// The config file resides one level above ABSPATH but is not part of another installation.
		if (
			file_exists( dirname( ABSPATH ) . '/' . $wp_config )
			&& ! file_exists( dirname( ABSPATH ) . '/wp-settings.php' )
		) {
			return dirname( ABSPATH ) . '/' . $wp_config;
		}
	 ...
	 * @param string $path
	 *
	 * @throws \StellarWP\PluginFramework\Exceptions\FilesystemException If the file not found.
	 *
	 * @return string
	 */
	protected function getConfigContent( $path ) {
		$content = $this->fs->get_contents( $path );

		if ( false === $content ) {
			throw new FilesystemException( sprintf( 'Unable to find %s.', $path ) );
		}
	...
	/**
	 * @throws \StellarWP\PluginFramework\Exceptions\FilesystemException If unable to write into file.
	 */
	protected function removeBlocksFromConfig() {
		$file    = $this->getConfigFilePath();
		$content = $this->getConfigContent( $file );
		foreach ( array_merge( ...array_values( $this->getPatterns() ) ) as $pattern ) {
			$content = preg_replace( $pattern, '', (string) $content );
		}

		if ( ! $this->fs->put_contents( $file, (string) $content, FS_CHMOD_FILE ) ) {
			throw new FilesystemException( sprintf( 'Unable to write to %s.', $file ) );
		}
	}
}
