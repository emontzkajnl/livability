...
require_once(SGPB_FORM_CLASSES_FIELDS.'Field.php');
    ...
    public function getAfterFieldHtml()
    {
        ...
        if (!empty($formObj)) {
            ...
            if (is_object($popup)) {
                ...
                if (empty($inputWidth)) {
                    $inputWidth =  $popup->getOptionValue('sgpb-contact-text-width');
                }
                $infoDivInlineStyles .= 'width: '.$inputWidth.';max-width: 100%;';
            }
        }

        $plainTextDescription = '<div class="sgpb-contact-text-checkbox-plaintext" style="'.$infoDivInlineStyles.'">'.@$settings['plainText'].'</div>';
