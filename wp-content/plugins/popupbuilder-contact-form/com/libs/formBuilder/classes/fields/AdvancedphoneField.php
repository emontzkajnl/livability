...
class AdvancedphoneField extends Field
{
    public function __construct()
    {
        $this->setDisplayName('Advanced Phone');
        $this->setType('advancedphone');
        $this->filters();
        $this->updateSettings();
    }
    ...
    {
        add_filter('sgpbContactNameValuesSeparator', array($this, 'separator'), 10, 2);
    }

    public function separator($separator, $fieldObj)
    {
        if ($fieldObj->getType() == 'advancedphone') {
            return ' - ';
        }
    ...
    public function getFieldRequiredSettings($index)
    {
        ...
        $settings['attrs']['name'] = 'sgpb-advanced-phone'.$uniqueIndex;
    ...
    {
        $html = parent::getGeneralEditSettingsRowsHtml('advancedphone');
	    ...
	    $html .= '<div class="sgpb-onOffSwitch">
					<input type="checkbox" class="sgpb-onOffSwitch-checkbox sgpb-settings-field sgpb-settings-required" id="jsFormEnableRequired_advancedphone"  data-key="required" value="">
					<label class="sgpb-onOffSwitch__label" for="jsFormEnableRequired_advancedphone">
    ...
    public function getFieldHtml()
    {
        ...
        $countriesCodes = $this->getCountriesPhoneCodes();
        $selectBoxName = $settings['attrs']['name'].'[countryCode]';

        $content = '<div class="sgpb-advanced-phone-field-wrapper">';
        $content .= PopupAdminHelper::createSelectBox($countriesCodes, '', array('name' => $selectBoxName, 'class'=>''));
        $settings['attrs']['name'] = $settings['attrs']['name'].'[number]';
        // it' for the js validation
        $settings['attrs']['data-field-type'] = 'customMultiple';

        $attrStr = $form->createAttrStr(@$settings['attrs']);
        $content .= '<input type="number" '.$attrStr.' placeholder="'.$placeHolder.'">';
        $content .= '</div><div class="sgpb-custom-field-error-message sgpb-subscription-plus-form-error-placement sgpb-error-message"></div>';

        return $content;
    }

    public function getCountriesPhoneCodes()
    {
        return array(
            '44' => "Selected>UK (+44)",
            '1' => 'USA (+1)',
