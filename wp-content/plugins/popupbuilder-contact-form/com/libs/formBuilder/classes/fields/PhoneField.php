...
class PhoneField extends Field
{
	...
	public function getFieldHtml()
	{
		...
		$settings['attrs']['class'][] = 'hide-spinner';
		$attrStr = $form->createAttrStr(@$settings['attrs']);
		$placeHolder = $settings['placeholder'];

		$styles = '<style>';
		$styles .= '.hide-spinner::-webkit-inner-spin-button {-webkit-appearance: none; margin: 0;}';
		$styles .= '.hide-spinner::-webkit-outer-spin-button {-webkit-appearance: none; margin: 0;}';
		$styles .= '.hide-spinner {-moz-appearance: textfield;}';
		$styles .= '</style>';

		return '<input type="number"'.$attrStr.' placeholder="'.$placeHolder.'" value="">'.$styles;
