...
if (class_exists('SGPBGetResponse')) {
	return false;
}
include_once 'getresponseApi.php';

class SGPBGetResponse extends SGPBProviderAbstract 
{
	...
	public static function api($integrationDetails)
	{
		...
		if (empty(self::$api)) {
			try {
				self::$api = new SGPBGetresponseApi($apiKey, array('debug' => true));
				self::$errors = array();
			} 
			catch (Exception $e) {
	...
	public function configureApiKey($integrationDetails) 
	{
		...
		$options = array(
			'api_key' => __('Please enter a valid GetResponse API key', SG_POPUP_TEXT_DOMAIN)
		);
	...
	private function validateApiKey($integrationDetails)
	{
		...
		if (is_wp_error($campaigns) || !$campaigns) {
			return false;
		}
