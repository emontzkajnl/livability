...
class FormCreator
{
	private $classPath = SGPB_FORM_CLASSES_FORMS;
	...
	public static function createSubscriptionFormObj($popupObj)
	{
		if (!is_object($popupObj)) {
			return '';
		}
		$currentFieldsJson = $popupObj->getOptionValue('sgpb-subscription-fields-json');
		$freeSavedOptions = $popupObj->getOptionValue('sgpb-subs-fields');
		// for back work compatibility
		if (empty($currentFieldsJson) && !empty($freeSavedOptions)) {
			$currentFieldsJson = FormCreator::createSavedObjFromFreeOptions($freeSavedOptions, $popupObj);
		}
		// for the old silver/gold/platinum version backward compatibility
		else if (empty($freeSavedOptions) && empty($currentFieldsJson)) {
			$savedFormFields = $popupObj->createFormFieldsData();
			$currentFieldsJson = FormCreator::createSavedObjFromFreeOptions($savedFormFields, $popupObj);
		}
	...
	public static function createSavedObjFromFreeOptions($freeOptions, $popupObj)
	{
		...
		// form Obj
		$formCreator = new FormCreator();
		$form = $formCreator->create('Subscription');
		$form->setPopupObj($popupObj);
		...
		foreach ($freeOptions as $fieldKey => $data) {
			...
			if (!empty($data['text'])) {
				$settings['gdprText'] = html_entity_decode($data['text']);
			}
