...
class Advanced_Ads_Selling_Admin_Placements {
	...
	public function show_placement_item_and_warning(){
		...
		if ( ! isset( $p_item['type'] ) ) {
			$warning = sprintf( __( 'When you press “Publish”, this ad will be published in the <strong>%2$s</strong> placement.', 'advanced-ads-selling' ), null, $placement_name );
			$do_import = true;
	...
	public static function get_placement_item( $p_slug ){
		...
		switch ( $_item[0] ) {
			...
			case Advanced_Ads_Select::GROUP :
				$groups = Advanced_Ads::get_instance()->get_model()->get_ad_groups( [ 'term_id' => $item_id ] );
				$group  = array_shift( $groups );

				if ( isset( $group->name ) ) {
					return [ 'type' => 'group', 'item_id' => $item_id, 'name' => $group->name ];
				}
				break;
		}
	}
