...
/* global advads, advanced_ads_pro, advanced_ads_sticky_items, advanced_ads_responsive, advanced_ads_sticky_settings, navigator */
...
document.addEventListener('aagam_empty_slot', function (ev) {
	...
	const wrapper = div.closest(
		'.' +
			document.body.classList.value.split('aa-prefix-')[1].split(' ')[0] +
			'sticky'
	);
		...
		if (advanced_ads_sticky_position_fixed_supported === false) {
			...
			setTimeout(function () {
				$stickyads.each(function (key, value) {
					...
					if (
						window.advanced_ads_sticky_items[$stickyad.attr('id')]
							.can_convert_to_abs
					) {
