...
class AdminHelper
{
	...
	{
		return file_exists(WP_PLUGIN_DIR.'/'.SGPB_WOO_PLUGIN_KEY);
	}
	...
	public static function isWoocommerceActive()
	{
		...
		return is_plugin_active(SGPB_WOO_PLUGIN_KEY);
	}
	...
	public static function allowToPublishPopup()
	{
		$allowToPublishPopup = true;

		if (!empty($_GET)) {
			if (isset($_GET['post_type']) && $_GET['post_type'] == SG_POPUP_POST_TYPE) {
				if (isset($_GET['sgpb_type']) && $_GET['sgpb_type'] == SGPB_POPUP_TYPE_RECENT_SALES) {
					$allowToPublishPopup = false;
				}
			}
		}

		return $allowToPublishPopup;
	}
	...
	public static function getStatusBySource($source = array())
	{
		$isWooActive = self::isWoocommerceActive();
		$isEddActive = self::isEddActive();
		$allStatuses = '';
		...
		else if (!$isWooActive && $isEddActive) {
			$allStatuses = edd_get_payment_statuses();
		}
		else {
			if ($source == SGPB_WOO_PLUGIN_SOURCE_KEY) {
				$allStatuses = wc_get_order_statuses();
			}
			else if ($source == SGPB_EDD_PLUGIN_SOURCE_KEY) {
				$allStatuses = edd_get_payment_statuses();
			}
		}
	...
	public static function getDefaultStatusBySource($source)
	{
		$status = 'completed';
