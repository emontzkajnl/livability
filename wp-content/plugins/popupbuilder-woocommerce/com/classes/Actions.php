...
class Actions
{
	public function __construct()
	{
		...
		$SGPB_DATA_CONFIG_ARRAY['woocommerce-special-events'] = $this->getBehaviorAfterSpecialEventsConfig();
	}

	public function wooSettingsView()
	{
		require_once SGPB_WOO_VIEWS_PATH.'mainView.php';
	}

	public function getBehaviorAfterSpecialEventsConfig()
	{
		$columns = array(
			'param' => __('Condition', SG_POPUP_TEXT_DOMAIN),
			'operator' => __('Parameter', SG_POPUP_TEXT_DOMAIN),
			'value' => __('Value', SG_POPUP_TEXT_DOMAIN)
		);
		...
		$params = array(
			'param' => array(
				SGPB_WOO_CONDITION_KEY => __('Shopping cart', SG_POPUP_TEXT_DOMAIN)
			),
			'operator' => array(
		...
		$attrs = array(
			...
			'product-is' => array(
				'htmlAttrs' => array(
					...
					'multiple' => 'multiple',
					'data-value-param' => SGPB_WOO_PRODUCT_POST_TYPE,
					'required' => 'required'
				),
				'infoAttrs' => array(
		...
		$config = array();
		$config['columns'] = apply_filters('sgPopupWooColumns', $columns);
		$config['columnTypes'] = apply_filters('sgPopupWooColumnTypes', $columnTypes);
		$config['paramsData'] = apply_filters('sgPopupWooParams', $params);
		$config['initialData'] = apply_filters('sgPopupWooInitialData', $initialData);
		$config['attrs'] = apply_filters('sgPopupWooAttrs', $attrs);
		$config['operators'] = apply_filters('sgPopupWooOperators', $operators);
		$config['specialDefaultOperator'] = apply_filters('sgPopupWooOperators', ' ');
